...
use utils::{
    ...
    {get_default_test_config, start_server_with_cfg_and_org_id},
};
...
async fn test_data() -> (OrganizationId, Vec<ApplicationId>, Vec<EndpointId>) {
    ...
    let (client, jh) =
        start_server_with_cfg_and_org_id(&get_default_test_config(), org_id.clone()).await;
...
async fn test_wiping_organization() {
    ...
    let db = web3hook_server::db::init_db(&cfg).await;
