...
pub fn get_default_test_config() -> ConfigurationInner {
    let _ = dotenv::dotenv();
    let cfg = web3hook_server::cfg::load().unwrap();

    cfg.as_ref().clone()
}

pub async fn start_server() -> (TestClient, tokio::task::JoinHandle<()>) {
    start_server_with_cfg(&get_default_test_config()).await
}

pub async fn start_server_with_cfg(
    cfg: &ConfigurationInner,
) -> (TestClient, tokio::task::JoinHandle<()>) {
    start_server_with_cfg_and_org_id(cfg, OrganizationId::new(None, None)).await
}

pub async fn start_server_with_cfg_and_org_id(
    cfg: &ConfigurationInner,
    ...
    let jh = tokio::spawn(web3hook_server::run_with_prefix(
